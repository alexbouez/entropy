 ...
 */
static void fast_mix(unsigned long s[4], unsigned long v1, unsigned long v2, unsigned int src)
{
	trace_printk("%lu, %lu, %u@", v1, v2, src);
	s[3] ^= v1;
...
void add_interrupt_randomness(int irq)
{
	...
	fast_mix(fast_pool->pool, entropy,
		 (regs ? instruction_pointer(regs) : _RET_IP_) ^ swab(irq), 0);
 ...
 */
static void add_timer_randomness(struct timer_rand_state *state, unsigned int num, unsigned int src)
{
	 ...
	 */
	if (in_hardirq()) {
		fast_mix(this_cpu_ptr(&irq_randomness)->pool, entropy, num, src);
	} else {
		spin_lock_irqsave(&input_pool.lock, flags);
		_mix_pool_bytes(&entropy, sizeof(entropy));
...
void add_disk_randomness(struct gendisk *disk)
{
	...
	add_timer_randomness(disk->random, 0x100 + disk_devt(disk), 2);
}
